...
bool ZipReader::containsFile(const QString &filename) const
{
    QList<FileInfo> files = fileInfoList();
    foreach (FileInfo zfi, files)
    {
        if(zfi.isFile)
        {
            QFileInfo fi(zfi.filePath);
            if(fi.fileName().toLower() == filename.toLower())
            {
                return true;
            }
        }
